...
parse cfg@(CFG _ _ s) input = filter isTerminated . cell (n-1,0) $ buildBoard cfg input
    where
...
partitions i j | i - j < 2 = 1
               | otherwise = i - j

updateCell :: (Show a, Eq a, Show b)=> [(a, (a, a))] -> Board a b -> (Int, Int, Int)  -> Board a b
updateCell rules board (i,j,k) = trace debug $ alter go (i, j) board
    where
        ...
        l_idx = (i,j+k)
        r_idx = (i-k, j)
        debug = "{" ++ show i ++ " " ++ show j ++ " " ++ show k ++ "} :: { " ++ show l_idx ++ " " ++ show r_idx ++ "} => "  ++ show left ++ " -- " ++ show right
